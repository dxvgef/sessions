...
func (this Manager) Use(req *http.Request, resp http.ResponseWriter) (session, error) {
	var sessObj session
	...
	if cookieValid {
		...
		// 将uuid结合key加密成sid
		if sidValue, err = encodeByBytes(strToByte(this.config.Key), strToByte(sessObj.ID)); err != nil {
			return sessObj, err
		}
